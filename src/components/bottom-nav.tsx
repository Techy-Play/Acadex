/**
 * @component BottomNav
 * @description Premium mobile bottom navigation bar with floating glassmorphic container,
 * tactile spring touch feedback, animated active indicator pills, and drop-up menus.
 */
"use client";
...
const manageSubItems = [
  ...
  { href: "/admin/user-requests",  label: "User Requests",     icon: <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg> },
...
const studentMoreItems = [
  ...
  { href: "/user/dashboard/requests",     label: "My Requests",  desc: "Ask or Suggest",     icon: <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg> },
  { href: "/user/dashboard/pending-work", label: "Pending Work", desc: "Deadlines",          icon: <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
  { href: "/user/dashboard/uploads",      label: "Uploads",      desc: "Your files",         icon: <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg> },
];
...
function PopupMenu({
  ...
  return (
    <>
      {open && (
        <div className="fixed inset-0 z-40 md:hidden bg-black/30 backdrop-blur-xs" onClick={onClose} />
      )}
      {open && (
        <div
          className={cn(
            "fixed bottom-20 z-50 w-64 rounded-2xl border border-border/60 bg-background/95 backdrop-blur-xl shadow-2xl p-2.5 animate-in slide-in-from-bottom-3 fade-in duration-200 md:hidden",
...
function NavTab({
  ...
  const inner = (
    <div className="flex flex-col items-center justify-center gap-1 w-full h-full py-1">
      <div
        className={cn(
          "relative flex items-center justify-center px-3.5 py-1 rounded-xl transition-all duration-200",
          active
...
export function BottomNav({
  ...
  if (variant === "admin") {
    ...
    return (
      <>
        ...
        <nav aria-label="Admin navigation" className="fixed bottom-3 left-3 right-3 z-40 md:hidden">
          <div className="flex items-center justify-around h-16 px-2 rounded-2xl border border-border/50 bg-background/85 backdrop-blur-xl shadow-2xl shadow-black/20">
  ...
  return (
    <>
      ...
      {/* Floating Glassmorphic Container */}
      <nav aria-label="Student navigation" className="fixed bottom-3 left-3 right-3 z-40 md:hidden">
        <div className="flex items-center justify-around h-16 px-1 rounded-2xl border border-border/50 bg-background/85 backdrop-blur-xl shadow-2xl shadow-black/20">
